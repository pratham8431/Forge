"""
Claude Haiku (generation) + Jina AI (embeddings) service for Forge RAG.
Jina's OpenAI-compatible endpoint lets us reuse the openai client for embeddings.
"""
import asyncio
import logging
    ...
    async def _batch(batch: List[str]) -> List[List[float]]:
        try:
            resp = await client.embeddings.create(
                model=RAGConfig.JINA_EMBEDDING_MODEL,
                input=batch,
                extra_body={"dimensions": RAGConfig.JINA_EMBEDDING_DIMS},
            )
            return [e.embedding for e in resp.data]
        except Exception as e:
            logger.error(f"Jina embedding batch error: {e}")
            return []
...
async def generate_answer(
    ...
    system_prompt = _build_prompt(query, relevant_docs, mode, document_url, file_extension, document_content, file_type)
    ...
    response = await client.messages.create(
        model=RAGConfig.CLAUDE_COMPLETION_MODEL,
