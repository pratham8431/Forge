          ...
          {me?.roles && me.roles.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-3">
              {me.roles.map((r) => (
                <span
                  key={r}
                  className="bg-[#0A84FF]/10 border border-[#0A84FF]/20 text-[#0A84FF] text-[12px] font-medium px-3 py-1 rounded-full"
                >
                  {r}
                </span>
              ))}
          ...
          {FEATURES.filter((f) =>
            ...
            const cardContent = (
              <>
                ...
                <div className="mt-4">
                  {f.available ? (
                    <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-[#30D158]/15 text-[#30D158]">
                      Available
                    </span>
                  ) : (
                    <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-white/[0.08] text-white/30">
                      Coming soon
                    </span>
                  )}
                </div>
            ...
            if (f.available && f.href) {
              return (
                <Link
                  key={f.title}
                  href={f.href}
                  className="bg-white/[0.04] border border-white/[0.06] rounded-2xl p-6 flex flex-col hover:bg-white/[0.07] hover:border-white/[0.12] hover:scale-[1.01] hover:shadow-[0_8px_32px_rgba(0,0,0,0.4)] hover:-translate-y-0.5 transition-all duration-200 cursor-pointer"
                >
                  {cardContent}
                </Link>
              )
            ...
            return (
              <div
                key={f.title}
                className="bg-white/[0.04] border border-white/[0.06] rounded-2xl p-6 flex flex-col opacity-60"
              >
                {cardContent}
              </div>
            )
...
const FEATURES: {
  ...
  {
    title: "RAG Search",
    description: "Search your internal knowledge base with AI.",
  ...
  },
  {
    ...
    description: "AI-powered incident triage and RCA.",
    permission: "incident:analyze",
    available: false,
    accent: "#FF453A",
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
  ...
  {
    title: "API Explorer",
    description: "Browse and test internal APIs.",
    permission: "api:explore",
    available: false,
    accent: "#FF9F0A",
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
